    ...
    public function taxonomyFilter($args = []) {
        ...
        if( wp_doing_ajax() ) :
            ...
            if( 'post' == $type ) :
                ...
                if( sanitize_text_field($_POST['area_of_interest']) && !empty(sanitize_text_field($_POST['area_of_interest'])) ) :
                    $term_ids=[];
                    $terms = get_terms([
                        'taxonomy'      => array( 'programs_cat' ),
                        'hide_empty'    => false,
                    ]);
                    $areas = explode(',', $_POST['area_of_interest']);
                    foreach ($terms as $term) {
                        foreach ($areas as $area) {
                            if (trim($term->name) == $area) {
